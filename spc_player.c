...
static void WriteVolumeToDsp(struct SpcPlayer *p, struct Channel *c, uint16 volume) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if ((c->pan_flag_with_phase_invert << i) & 0x80)
      t = -t;
    Dsp_Write(p, (enum DspReg)(V0VOLL + i + c->index * 16), t);
...
static void WritePitch(struct SpcPlayer *p, struct Channel *c, uint16 pitch) {
  ...
  if (!(p->cur_chan_bit & p->is_chan_on)) {
    ...
    Dsp_Write(p, (enum DspReg)(reg + V0PITCHL), t & 0xff);
    Dsp_Write(p, (enum DspReg)(reg + V0PITCHH), t >> 8);
  }
}
...
static void Channel_SetInstrument(struct SpcPlayer *p, struct Channel *c, uint8 instrument) {
  ...
  if (ip[0] & 0x80) {
    ...
    Dsp_Write(p, (enum DspReg)(reg + V0SRCN), 0);
  } else {
...
struct DspRegWriteHistory my_write_hist;
