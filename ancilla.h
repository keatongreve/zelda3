...
struct ProjectSpeedRet Bomb_ProjectSpeedTowardsPlayer(int k, uint16 x, uint16 y, uint8 vel);
void Boomerang_CheatWhenNoOnesLooking(int k, struct ProjectSpeedRet *pt);
...
void Ancilla_Main();
struct ProjectSpeedRet Ancilla_ProjectReflexiveSpeedOntoSprite(int k, uint16 x, uint16 y, uint8 vel);
...
bool Ancilla_ReturnIfOutsideBounds(int k, struct AncillaOamInfo *info);
...
void Ancilla_SetupHitBox(int k, struct SpriteHitBox *hb);
struct ProjectSpeedRet Ancilla_ProjectSpeedTowardsPlayer(int k, uint8 vel);
struct PairU8 Ancilla_IsRightOfLink(int k);
struct PairU8 Ancilla_IsBelowLink(int k);
...
struct OamEnt *AncillaDraw_Explosion(struct OamEnt *oam, int frame, int idx, int idx_end, uint8 r11, int x, int y);
...
struct OamEnt *AncillaDraw_EtherBlitzBall(struct OamEnt *oam, const struct AncillaRadialProjection *arp, int s);
struct OamEnt *AncillaDraw_EtherBlitzSegment(struct OamEnt *oam, const struct AncillaRadialProjection *arp, int s, int k);
void AncillaDraw_EtherBlitz(int k);
void AncillaDraw_EtherOrb(int k, struct OamEnt *oam);
...
struct OamEnt *Ancilla_ReceiveItem_Draw(int k, int x, int y);
...
void AncillaDraw_GTCutsceneCrystal(struct OamEnt *oam, int x, int y);
void GTCutscene_ActivateSparkle();
struct OamEnt *GTCutscene_SparkleALot(struct OamEnt *oam);
...
struct Point16U Sparkle_PrepOamFromRadial(struct AncillaRadialProjection p);
...
void Ancilla_SetupBasicHitBox(int k, struct SpriteHitBox *hb);
...
void Ancilla_PrepOamCoord(int k, struct Point16U *info);
void Ancilla_PrepAdjustedOamCoord(int k, struct Point16U *info);
void Ancilla_SetOam_XY(struct OamEnt *oam, uint16 x, uint16 y);
uint8 Ancilla_SetOam_XY_safe(struct OamEnt *oam, uint16 x, uint16 y);
bool Ancilla_CheckLinkCollision(int k, int j, struct CheckPlayerCollOut *out);
bool Hookshot_CheckProximityToLink(int x, int y);
bool Ancilla_CheckForEntranceTrigger(int what);
void AncillaDraw_Shadow(struct OamEnt *oam, int k, int x, int y, uint8 pal);
void Ancilla_AllocateOamFromRegion_B_or_E(uint8 size);
struct OamEnt *Ancilla_AllocateOamFromCustomRegion(struct OamEnt *oam);
struct OamEnt *HitStars_UpdateOamBufferPosition(struct OamEnt *oam);
bool Hookshot_ShouldIEvenBotherWithTiles(int k);
struct AncillaRadialProjection Ancilla_GetRadialProjection(uint8 a, uint8 r8);
...
bool Bomb_CheckUndersideSpriteStatus(int k, struct Point16U *out_pt, uint8 *out_r10);
